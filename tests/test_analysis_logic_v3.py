...
def test_text_scenarios_use_the_same_normalized_levels_as_chart():
    candles = _flat_candles(price=4120.0)
    data = {
        "chart_readable": True,
        "candles": candles,
        ...
        "image_price_low": 4117.0,
        "support_levels": [
            {"price": 4119.25, "strength": 80, "touches": 3},
            {"price": 4118.10, "strength": 70, "touches": 2},
        ],
        "resistance_levels": [
            {"price": 4120.85, "strength": 82, "touches": 3},
            {"price": 4122.15, "strength": 74, "touches": 2},
        ],
        "entry": 4120.0,
        "stop_loss": 4118.8,
        "target_1": 4121.2,
        ...
        "pattern_type": "لا يوجد",
        "pattern_confidence": 0,
        "bullish_scenario": "مقاومة مختلفة 9999",
        "bearish_scenario": "دعم مختلف 1111",
    }
    summary = {
        "direction": "عرضي",
        "alignment": 50,
    ...
    result = _validate_analysis(data, summary)
    r1 = result["resistance_levels"][0]["price"]
    r2 = result["resistance_levels"][1]["price"]
    s1 = result["support_levels"][0]["price"]
    s2 = result["support_levels"][1]["price"]
    assert f"{r1:.2f}" in result["bullish_scenario"]
    assert f"{r2:.2f}" in result["bullish_scenario"]
    assert f"{s1:.2f}" in result["bearish_scenario"]
    assert f"{s2:.2f}" in result["bearish_scenario"]
    assert "9999" not in result["bullish_scenario"]
    assert "1111" not in result["bearish_scenario"]
    assert result["level_source_policy"] == "single_normalized_levels_for_chart_and_text"
